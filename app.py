...
# --- TEMPORARY DIAGNOSTIC: show exactly what st.secrets contains --------
# This block is here only to find out why the key isn't reaching the app.
# It never prints the actual key value, only whether it's present and
# how long it is (to catch empty-string / whitespace-only values).
with st.sidebar.expander("🔧 DEBUG: Secrets Diagnostic (temporary)"):
    try:
        secret_keys_found = list(st.secrets.keys())
        st.write("Keys visible in st.secrets:", secret_keys_found)
        for _k in ("GOOGLE_API_KEY", "GEMINI_API_KEY"):
            if _k in st.secrets:
                val = st.secrets[_k]
                st.write(f"{_k}: present, length={len(val)}, starts_with={val[:6]!r}")
            else:
                st.write(f"{_k}: NOT FOUND in st.secrets")
    except Exception as diag_error:
        st.write("st.secrets raised an exception:", repr(diag_error))
# -------------------------------------------------------------------------
...
with st.sidebar.expander("🔧 DEBUG: os.environ after sync (temporary)"):
    st.write("GOOGLE_API_KEY in os.environ:", "GOOGLE_API_KEY" in os.environ)
    st.write("GEMINI_API_KEY in os.environ:", "GEMINI_API_KEY" in os.environ)
