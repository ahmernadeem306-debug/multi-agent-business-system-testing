...
# calling anything else (even st.sidebar.expander) before this breaks the
# app's layout. This restores the original structure exactly.
st.set_page_config(
...
# --- Silent diagnostic: goes to Streamlit Cloud's backend logs only -----
# (Manage app -> logs), never rendered in the UI, so it can't affect the
# app's layout or structure. Safe to leave in permanently if you want.
try:
    print("[BIZAGENT-DIAG] st.secrets keys visible:", list(st.secrets.keys()))
    for _k in ("GOOGLE_API_KEY", "GEMINI_API_KEY"):
        if _k in st.secrets:
            _v = st.secrets[_k]
            print(f"[BIZAGENT-DIAG] {_k}: present, length={len(_v)}, starts_with={_v[:6]!r}")
        else:
            print(f"[BIZAGENT-DIAG] {_k}: NOT FOUND in st.secrets")
except Exception as _diag_error:
    print("[BIZAGENT-DIAG] st.secrets raised an exception:", repr(_diag_error))

# --- Ensure API keys reach os.environ no matter what --------------------
# Streamlit Cloud is supposed to auto-expose root-level secrets.toml keys
# as environment variables; this makes it explicit and guaranteed instead
# of relying on that automatic behavior.
...
print("[BIZAGENT-DIAG] GOOGLE_API_KEY in os.environ after sync:", "GOOGLE_API_KEY" in os.environ)
print("[BIZAGENT-DIAG] GEMINI_API_KEY in os.environ after sync:", "GEMINI_API_KEY" in os.environ)
# -------------------------------------------------------------------------
